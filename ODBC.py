...
conn = pyodbc.connect('DSN=Mayank-DBMS;UID=root;PWD=mayank')
...
query_6 = "SELECT * FROM SOCIETY WHERE TotalSeats > ?;"
cursor.execute(query_6, (N,))
print("\nQuery 6 Results:")
for row in cursor.fetchall():
    print(row)  
...
query_7 = "UPDATE SOCIETY SET MentorName = ? WHERE SocID = ?;"
cursor.execute(query_7, (MentorName, SocID,))
conn.commit()  # Commit the transaction to make the changes permanent

# Fetch and print the updated row
cursor.execute("SELECT * FROM SOCIETY WHERE SocID = ?", (SocID,))
updated_row = cursor.fetchone()
if updated_row:
    print("\nUpdated Society Details:")
    print("Society ID:", updated_row[0])
    print("Society Name:", updated_row[1])
    print("Mentor Name:", updated_row[2])
    # Print other columns as needed
else:
    print("\nNo society found with the specified ID:", SocID)
